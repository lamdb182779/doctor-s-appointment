...
import "../../../styles/Guest/Doctors/BookingNow.scss"
import { useState, useRef, useEffect } from "react"
import moment from "moment"
import "moment/locale/vi"
import { timeframe, dayArray } from "../../../constants/variables/schedule"
import { Button, Col, Row, Image, Dropdown, FloatingLabel, Form } from "react-bootstrap"
...
const BookingNow = (props) => {
    ...
    const handleDay = (date) => {
        if (date !== day) {
            scheduleRef.current.classList.add("hide")
            if (data?.length > 0) {
                appointmentRef.current.classList.add("scroll")
            }
            setTimeout(() => {
                setIndex(0)
                setDay(date)
                scheduleRef.current.classList.remove("hide")
                if (data?.length > 0) {
                    appointmentRef.current.classList.remove("scroll")
                }
            }, [200])
        }
    }
    ...
    return (
        <>
            <Col xs={2} className="text-center d-flex align-items-center">
                <div>
                    ...
                    <Button variant="outline-primary" onClick={event => props.handleInfo(event)}>
                        Xem thông tin
                    </Button>
                </div>
            </Col>
            <Col xs={data?.length > 0 ? 5 : 10} className="p-0">
            ...
            <Col xs={5} className="d-flex align-items-center appointment" ref={appointmentRef} >
                {loading === false ?
                    <>
                        {data?.length > 0 ?
                            <>
                                <div>
                                    ...
                                    <Row>
                                        <Col>
                                            <div className="">
                                                <Button variant="danger" onClick={() => handleReset()} className="mt-3 w-100" size="sm">
                                                    Làm mới lịch hẹn
                                                </Button>
                                                <Button onClick={() => handleBooking()} className="mt-2 w-100" size="sm">
                                                    Đặt lịch hẹn
                                                </Button>
                                            </div>
                                        </Col>
